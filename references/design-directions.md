...
已有用户原型时优先提取其规则，不另选主题。没有参考时可按需查看 [style-gallery.html](style-gallery.html)：它是一个服务查询场景的离线演示，不是十种风格的完整实现，也不是正式验收基线。禁止浏览器或不需要视觉预览时不打开。设计方向须落实为 [设计与交互规范](design-spec.md) 中的变量、组件和状态，不能只挑一个品牌名。
...
1. 已有表时先用当前 MCP 核实字段与选项。新应用尚未建表时可使用明确标为“提议、未绑定”的字段和假数据，不编造真实 key，也不要求先提供凭证才能开始设计。
